...
$sql = "DELETE FROM SudokuTemp";
if ($con->query($sql) === FALSE) {
    echo "Error deleting record: " . $con->error;
}
// Get sudoku data from database
$result = mysqli_query($con,"SELECT * FROM Sudoku_Easy order by rand() limit 1");
while ($row = mysqli_fetch_assoc($result)){
    ...
    $sql = "INSERT INTO SudokuTemp (TL,TM,TR,ML,MM,MR,BL,BM,BR) VALUES ('$u[0]','$u[1]','$u[2]','$u[3]','$u[4]','$u[5]','$u[6]','$u[7]','$u[8]')";
    if ($con->query($sql) === FALSE) {
        echo "Error: " . $sql . "<br>" . $con->error;
    }
...
<div class="container">
  ...
  <h1>Sudoku Board</h1>
  <h2>Generate a random Sudoku board. If you find one that you like, press start. </h2>
  <h3>
    <a href="sudoku.php" class="button">Start</a>
    <a href="sudokuMenu.php" class="button">ReRoll</a>
  </h3>
